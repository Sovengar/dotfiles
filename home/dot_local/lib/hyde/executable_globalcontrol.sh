...
export HYDE_STATE_FILE="$HYPR_STATE_HOME/hyde.conf"
...
state_file_for_var() {
    case "$1" in
    HYDE_THEME | PALETTE_SOURCE) echo "$HYDE_STATE_FILE" ;;
    HYPR_ANIMATION) echo "$HYPR_STATE_HOME/animation.conf" ;;
    HYPR_WORKFLOW) echo "$HYPR_STATE_HOME/workflow.conf" ;;
    HYPR_SHADER) echo "$HYPR_STATE_HOME/shader.conf" ;;
    HYPRLOCK_LAYOUT) echo "$HYPR_STATE_HOME/hyprlock.conf" ;;
    WEATHER_*) echo "$HYPR_STATE_HOME/weather.conf" ;;
    WAYBAR_*) echo "$WAYBAR_STATE_FILE" ;;
    *) echo "$HYPR_STATE_HOME/state" ;;
    esac
}
...
export_hyde_config() {
    source_if_exists "$HYDE_STATE_FILE"
    source_if_exists "$HYPR_STATE_HOME/animation.conf"
    source_if_exists "$HYPR_STATE_HOME/workflow.conf"
    source_if_exists "$HYPR_STATE_HOME/hyprlock.conf"
    source_if_exists "$HYPR_STATE_HOME/shader.conf"
    source_if_exists "$HYPR_STATE_HOME/weather.conf"
...
export HYDE_THEME HYDE_THEME_DIR HYDE_THEME_WALL WALLBASH_DIRS wallbashDirs PALETTE_SOURCE
