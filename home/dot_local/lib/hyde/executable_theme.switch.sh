...
sanitize_hypr_theme() {
    ...
    generate_waybar_theme "$buffer_file" "${WAYBAR_STATE_HOME:-${XDG_STATE_HOME:-$HOME/.local/state}/waybar}/theme.conf"
    rm -f "$buffer_file"
}
hypr_theme_value() {
    local input_file="$1"
...
if [[ -r $HYPRLAND_CONFIG ]]; then
    ...
    export GTK_THEME ICON_THEME CURSOR_THEME CURSOR_SIZE FONT FONT_SIZE CODE_THEME
...
theme_color_scheme="${COLOR_SCHEME:-$(get_hyprConf COLOR_SCHEME)}"
theme_color_scheme="${theme_color_scheme:-prefer-dark}"
hyde_state_dir="${HYPR_STATE_HOME:-${XDG_STATE_HOME:-$HOME/.local/state}/hypr}"
mkdir -p "$hyde_state_dir"
{
    printf '%s\n' 'return {'
    printf '    HYDE_THEME = %s,\n' "$(lua_string "$HYDE_THEME")"
    printf '    PALETTE_SOURCE = %s,\n' "$(lua_string "${PALETTE_SOURCE:-theme}")"
    printf '    GTK_THEME = %s,\n' "$(lua_string "${GTK_THEME:-default}")"
    printf '    COLOR_SCHEME = %s,\n' "$(lua_string "${theme_color_scheme}")"
    printf '    ICON_THEME = %s,\n' "$(lua_string "${ICON_THEME:-default}")"
    printf '    CURSOR_THEME = %s,\n' "$(lua_string "${CURSOR_THEME:-default}")"
    printf '    CURSOR_SIZE = %s,\n' "$(lua_number "${CURSOR_SIZE:-24}")"
    printf '    FONT = %s,\n' "$(lua_string "${FONT:-Cantarell}")"
    printf '    FONT_SIZE = %s,\n' "$(lua_number "${FONT_SIZE:-10}")"
    printf '    DOCUMENT_FONT = %s,\n' "$(lua_string "${DOCUMENT_FONT:-Cantarell}")"
    printf '    DOCUMENT_FONT_SIZE = %s,\n' "$(lua_number "${DOCUMENT_FONT_SIZE:-10}")"
    printf '    MONOSPACE_FONT = %s,\n' "$(lua_string "${MONOSPACE_FONT:-CaskaydiaCove Nerd Font Mono}")"
    printf '    MONOSPACE_FONT_SIZE = %s,\n' "$(lua_number "${MONOSPACE_FONT_SIZE:-9}")"
    printf '    BAR_FONT = %s,\n' "$(lua_string "${BAR_FONT:-JetBrainsMono Nerd Font}")"
    printf '    MENU_FONT = %s,\n' "$(lua_string "${MENU_FONT:-Cantarell}")"
    printf '    NOTIFICATION_FONT = %s,\n' "$(lua_string "${NOTIFICATION_FONT:-Mononoki Nerd Font Mono}")"
    printf '    CODE_THEME = %s,\n' "$(lua_string "${CODE_THEME:-Wallbash}")"
    printf '    SDDM_THEME = %s,\n' "$(lua_string "${SDDM_THEME:-Wallbash}")"
    printf '    TERMINAL = %s,\n' "$(lua_string "${TERMINAL:-wezterm}")"
    printf '%s\n' '}'
} > "$hyde_state_dir/startup_vars.lua"
print_log -sec "theme" -stat "generated" "startup_vars.lua"
