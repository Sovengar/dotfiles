...
if [[ $HYPRLAND_CONFIG == *.lua ]]; then
    HYDE_HYQ_CONFIG="${HYPR_GENERATED_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/hypr/generated}/hyprland.finale.conf"
    legacy_hyq_config="${XDG_CONFIG_HOME:-$HOME/.config}/hypr/autogenerated_files/hyprland.finale.conf"
    [[ -r $HYDE_HYQ_CONFIG || ! -r $legacy_hyq_config ]] || HYDE_HYQ_CONFIG="$legacy_hyq_config"
fi

if [[ -r $HYDE_HYQ_CONFIG ]] && command -v "hyq" &> /dev/null; then
    eval "$(hyq "$HYDE_HYQ_CONFIG" --source --export env \
        -Q 'hyde:gtk-theme' \
        -Q 'hyde:color-scheme' \
        -Q 'hyde:icon-theme' \
        -Q 'hyde:cursor-theme' \
