...
cat <<WALLBASH >"$hypr_generated_dir/wallbash.conf"
...
# // ----------------------------
WALLBASH
if grep -q "#//---Wallbash mode enabled---" "$hypr_generated_dir/wallbash.conf"; then
    sed -i '/#\/\/---Wallbash mode enabled---/,$d' "$hypr_generated_dir/wallbash.conf"
fi
if [[ $enableWallDcol -gt 0 ]]; then
    cat <<ON_WALLBASH >>"$hypr_generated_dir/wallbash.conf"
