...
fn_select() {
    ...
    layout_dir="$confDir/hypr/hyprlock/layouts"
...
generate_conf() {
    local path="${1:-$confDir/hypr/hyprlock/layouts/theme.conf}"
    local target_file="${2:-${XDG_STATE_HOME:-$HOME/.local/state}/hypr/hyprlock}"
    local entrypoint="${confDir:-${XDG_CONFIG_HOME:-$HOME/.config}}/hypr/hyprlock.conf"

    if [ -n "${2:-}" ]; then
        {
            printf '# Generated preview by hyprlock.sh; do not edit manually.\n'
            printf '$LAYOUT_PATH=%s\n' "$path"
            awk '
                /Dynamic state generated by hyprlock.sh/ { skip = 1; next }
                skip && /# hyprlang noerror false/ { skip = 0; next }
                skip { next }
                { print }
            ' "$entrypoint"
        } >"$target_file"
...
# Generated by hyprlock.sh; do not edit manually.
\$LAYOUT_PATH=$path
CONF
}
...
fn_reload() {
    ...
    fn_profile
    reload_hyprlock
    notify-send -i "system-lock-screen" "Hyprlock state regenerated" "Layout: $layout"
}

ensure_lockscreen_bg_exist() {
    if [ ! -f "$HYPRLOCK_STATE_HOME/wallpaper.png" ]; then
        print_log -sec "hyprlock" -stat "setting" " $HYPRLOCK_STATE_HOME/wallpaper.png"
        "${LIB_DIR}/hyde/wallpaper.sh" -s "$(readlink "$HYDE_THEME_WALL")" --backend hyprlock
    fi
}
...
argparse "--reload,-r" "" "Regenerates hyprlock state using HYPRLOCK_LAYOUT (defaults to HyDE)"
