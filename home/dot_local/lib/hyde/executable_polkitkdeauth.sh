...
# This script starts the official hyprpolkitagent user service first, then falls
# back to the first available Polkit agent from a list of possible locations.

if command -v systemctl >/dev/null 2>&1 && systemctl --user list-unit-files hyprpolkitagent.service >/dev/null 2>&1; then
  if systemctl --user start hyprpolkitagent.service >/dev/null 2>&1; then
    echo "Started: hyprpolkitagent.service"
    exit 0
  fi
fi
