...
if [[ $- == *i* && -o zle && -t 0 ]]; then
  ...
  zle -N fdx-file-widget fdx_file_widget_zsh

  bindkey '!' expand-bang-history
  bindkey '$' expand-last-argument
  bindkey '*' expand-pipe-fzf-trigger
  bindkey '^F' fdx-file-widget
  # Disable the global fzf Ctrl+T file picker: file insertion is handled by Ctrl+F.
  # This does not affect Ctrl+T inside fdx, where it cycles the f/d/all type filter.
