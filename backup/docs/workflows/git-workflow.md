...
Terminal diff viewer para revisar cambios locales (Working Directory + Staging Area) y añadir comentarios inline para la IA. Construido sobre OpenTUI, con sidebar de navegación multi-archivo, modo split/stack responsivo, y anotaciones inline de agentes/AI.

**Modos de uso**:
- `hunk diff` — revisar working tree (Working Directory + Staging Area, incluye untracked files por defecto)
...
|--------|-------------|------------|-------|
| LazyGit flotante | LazyGit | `SUPER+G` | pypr toggle, flota sobre cualquier workspace |

### Capas de acceso VCS

```
Hyprland (SUPER+G/D/W)     ← Flotante, siempre disponible
Fish (lgit/ldk/lzn)        ← Terminal, rápido
LazyGit interior (1-5, c, P) ← Gestión detallada dentro de LazyGit
Neovim (<leader>gv/gD)    ← Diff y historial profundo
```