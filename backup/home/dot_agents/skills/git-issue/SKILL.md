...
name: git-issue
description: >
  Generic issue creation workflow for Git providers (GitHub, GitLab, Bitbucket).
  Trigger: When creating issues, bug reports, or feature requests.
---
...
> **Note:** Detect the provider automatically or ask if unclear. See supported providers below.

---

## Supported Providers

| Provider | CLI Command | Template Path | Labels System |
|----------|-------------|---------------|---------------|
| GitHub | `gh issue create` | `.github/ISSUE_TEMPLATE/` | GitHub labels |
| GitLab | `glab issue create` | `.gitlab/issue_templates/` | GitLab labels |
| Bitbucket | `bq issue create` | `.bitbucket/ISSUE_TEMPLATES/` | Bitbucket labels |
...
```
1. Detect provider (GitHub, GitLab, Bitbucket)
2. Search existing issues for duplicates
3. Choose the correct template (Bug Report or Feature Request)
4. Fill in ALL required fields
5. Submit → provider-specific labels applied
...
> Use the section corresponding to your provider. If not specified, ask the user.

---

## GitHub-specific

### Critical Rules (GitHub)
...
### Maintainer Approval Workflow (GitHub)
...
> **Coming soon.** When GitLab support is needed, add:
> - GitLab CLI (`glab`) commands
> - GitLab template paths
> - GitLab labels and approval workflow

### Placeholder Commands

```bash
# Search existing issues (GitLab)
glab issue list --search "keyword"

# Create bug report (GitLab)
glab issue create --title "fix(scope): description" --description "..."

# Create feature request (GitLab)
glab issue create --title "feat(scope): description" --description "..."
```
...
> **Coming soon.** When Bitbucket support is needed, add:
> - Bitbucket CLI (`bq`) commands
> - Bitbucket template paths
> - Bitbucket labels and approval workflow

### Placeholder Commands

```bash
# Search existing issues (Bitbucket)
bq issue list --search "keyword"

# Create bug report (Bitbucket)
bq issue create --title "fix(scope): description" --description "..."

# Create feature request (Bitbucket)
bq issue create --title "feat(scope): description" --description "..."
```
...
To add a new provider (e.g., Gitea, Azure DevOps):

1. Add the provider to the **Supported Providers** table
2. Create a new section `## {Provider}-specific`
3. Include:
   - CLI commands
   - Template paths
   - Labels system
...
4. Update the **Decision Tree** if needed
