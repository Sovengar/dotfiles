...
| Generated Hypr state | HyDE engine for now | Generated Hypr artifacts live in `$XDG_STATE_HOME/hypr/generated`, not in `.config`. They are runtime state and stay out of chezmoi. |
| Wallbash templates | User / chezmoi | `~/.local/share/wallbash` is now the owned template source that tells HyDE where to write generated app/theme outputs. |
...
| Generated Hypr files belong in `.config`. | They are state and now live under `$XDG_STATE_HOME/hypr/generated`. |
...
| Hypr generated state | Owned boundary, HyDE generated | HyDE writes `$XDG_STATE_HOME/hypr/generated/*`; `hyprland/scripts/*` reads it; stable config no longer tracks `autogenerated_files`. |
| Wallbash templates | Owned | `~/.local/share/wallbash/**` is now in chezmoi, including Hypr generated-state targets. |
