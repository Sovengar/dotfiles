...
Current boundary: HyDE still generates colors/themes, but Hypr-generated artifacts are state under `$XDG_STATE_HOME/hypr`. The stable Lua config under `~/.config/hypr/hyprland/` consumes that state through `hypr/scripts/*` and applies config through `hyprland/style/*`.
...
| `~/.local/share/hyde/wallbash/**` | Owned dcol templates and post-process scripts tracked by chezmoi. |
...
|------------|----------|----------|
| `$XDG_STATE_HOME/hypr/wallbash.conf` | `color/hypr.sh` | `hypr/scripts/wallbash_metadata_loader.lua` updates HyDE variables. |
| `$XDG_STATE_HOME/hypr/hyprland.theme.lua` | `theme.switch.sh` | `hypr/scripts/generated_theme_loader.lua`, then `style/theme.lua`. |
| `$XDG_STATE_HOME/hypr/colors.conf` | `~/.local/share/hyde/wallbash/always/hyprcolors.dcol` | `hypr/scripts/generated_theme_loader.lua`, then `style/theme.lua`. |
| `$XDG_STATE_HOME/hypr/animations.theme.conf` | `~/.local/share/hyde/wallbash/theme/animations.dcol` | `hypr/scripts/animation_state_loader.lua`, then `style/animations.lua`. |
| `$XDG_STATE_HOME/hypr/shaders.conf` | `shaders.sh` | `hypr/scripts/shader_state_loader.lua`, then `style/shaders.lua`. |
| `$XDG_STATE_HOME/hypr/waybar.theme.conf` | `theme.switch.sh` | `wbarstylegen.sh`. |
| `$XDG_STATE_HOME/hypr/metadata.conf` | `hypr/metadata_generator.lua` | Snapshot for tooling/debugging; `color/dconf.sh`, `globalcontrol.sh` fallback metadata lookups. |

`hypr/scripts/generated_paths.lua` owns the generated-state path helper and temporary legacy fallback to the old `.config/hypr/autogenerated_files` location.

## Generated And Partially Rewritten Files

| File | Behavior |
|------|----------|
| `$XDG_STATE_HOME/hypr/colors.conf` | Fully generated state. |
| `$XDG_STATE_HOME/hypr/hyprland.theme.lua` | Fully generated state from theme. |
| `$XDG_STATE_HOME/hypr/wallbash.conf` | Fully generated state; bridge read by `wallbash_metadata_loader.lua`. |
...
| 3 | Own theme dirs and dcol templates first; do not rewrite generator yet. | Done: `~/.local/share/hyde/wallbash` is the canonical Wallbash source tracked in chezmoi; app-specific templates/scripts formerly under `~/.config/hyde/wallbash` were merged here. No tracked Wallbash files remain under `~/.config/hyde`. |
