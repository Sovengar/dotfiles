...
Kitty, fish, and zsh have moved toward owned config structure: kitty keeps stable config in chezmoi and delegates generated colors to HyDE; fish and zsh use owned `conf.d/*` modules.
...
| `~/.config/fish/**` | ✅ Yes | Fish startup is split into owned modules; HyDE aliases/completion are intentional runtime calls |
